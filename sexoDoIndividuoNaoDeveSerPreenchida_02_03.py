...
def sexo_do_individuo_nao_deve_ser_preenchido():
    try:
        ...
        cur.execute('''
           SELECT v.Id, v.NomeDoIndividuo, v.SexoDoIndividuo, v.TipoImovel FROM VisitaDomiciliar v 
           WHERE v.Id IN ( 
           SELECT i.`Cadastro_Id` FROM `LoteIntegracao` i
           LEFT JOIN `Lote` l ON (l.`Id` = i.`Lote_Id`
           )
           WHERE i.`Erros` 
           LIKE '%O campo \'SexoDoIndividuo\' não deve ser preenchida quando o \'Tipo Imóvel\' for igual a 
           (02 comércio, 03 terreno baldio, 04 Ponto Estratégico, 05 Escola, 06 Creche ou 12 Estabelecimento religioso)!.%')
        ''')
